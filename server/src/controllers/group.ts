...
export const createGroup = asyncHandler(
  async (req: Request<{}, {}, { groupName: string; userIds: number[] }>, res: Response, next: NextFunction) => {
    const userId = req.user.id;
    const { userIds, groupName } = req.body;

    let err: string | null = null;
    if (!groupName) err = 'Group name is required';
    else if (!userIds || userIds.length === 0) err = "Can't create an empty group. At least one member is required";
    ...
    const friendIds = new Set(friends.map((f) => [f.friendId, f.userId]).reduce((acc, curr) => acc.concat(curr), []));

    userIds.forEach((id) => {
      if (!friendIds.has(id)) throw new Error(`User with id of '${id}' is not a friend`);
    });

    const roomId = await roomUOW.newRoom(RoomType.Group, groupName);
    await participantsUOW.newParticipants(
      [
        ...userIds.filter((id) => id != userId).map((id) => ({ id, role: Role.Member })),
...
// @desc Delete a group
// @access  Onwer only
export const deleteGroup = asyncHandler(async (req: Request<{ id: string }>, res: Response, next: NextFunction) => {
  const userId = req.user.id;
  ...
  if (participant.role !== Role.Owner) throw new Error('Only owner can delete the group');
...
// @desc Leave a group
// @access  Private/user
export const leaveGroup = asyncHandler(async (req: Request<{ id: string }>, res: Response, next: NextFunction) => {
  const userId = req.user.id;
  let roomId = Number(req.params.id);
  if (isNaN(roomId)) throw new Error(`'${roomId}' is not a valid roomId`);
  ...
  const participant = room.participants.find((p) => p.userId == userId);
  if (!participant) throw new Error("User isn't part of the room");
  if (participant.role === Role.Owner) throw new Error("Owner can't leave the gorup.");

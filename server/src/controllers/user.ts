...
export const login = asyncHandler(
  async (req: Request<{}, {}, { email: string; password: string }>, res: Response, next: NextFunction) => {
    ...
    if (!user) return next(new ResponseError('Incorrect email or password', 400));

    const userInfo = user.get();
    let secret = process.env.JWT_SECRET;
    if (secret == undefined) return next(new ResponseError('Missing `JWT_SECRET` env. variable', 500));

    let token = jwt.sign({ id: userInfo.id }, process.env.JWT_SECRET as Secret, {
      expiresIn: '7d',
    });
...
export const getUsers = asyncHandler(
  async (
    ...
    const friendsList = parseQuery<Array<{ userId: number; friendId: number; status: FriendshipStatus }>>(friends);
...
// @access  Private/user
export const getFriends = asyncHandler(async (req: Request, res: Response, _next: NextFunction) => {
  ...
  res.status(200).json(entries.map(({ roomId, user }) => ({ roomId, ...user })));
});

  ...
  async addFriend(friendId: number) {
    ...
    await this.errorHandler(
      async () => {
        ...
        if (userId === friendId) throw new Error("User can't add himself as a friend");
        ...
        await friendUOW.newFriend(sender.id, receiver.id, FriendshipStatus.Pending);
        const isSent = await this.app.notificationRepo.isFriendshipRequestAlreadySent(sender.id, receiver.id);
  ...
  async acceptFriend(friendId: number) {
    ...
    await this.errorHandler(
      async () => {
        ...
        const [user, request] = await Promise.all([userUOW.getById(userId), friendUOW.getFriendRequest(friendId)]);
  ...
  async notifyFriends(userId: number) {
    ...
    await this.errorHandler(async () => {
      ...
      socket.broadcast
        .to(
          friends
            .map((f) => [getUserPrivateRoom(f.userId), getUserPrivateRoom(f.friendId)])
